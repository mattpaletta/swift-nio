...
RUN mkdir -p $HOME/.scripts && wget -q https://raw.githubusercontent.com/apple/swift/master/utils/symbolicate-linux-fatal -O $HOME/.scripts/symbolicate-linux-fatal && chmod 755 $HOME/.scripts/symbolicate-linux-fatal && echo 'export PATH="$HOME/.scripts:$PATH"' >> $HOME/.profile && echo 'export PATH="/root/.swift/usr/bin:$PATH"' >> $HOME/.profile
